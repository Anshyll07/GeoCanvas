...
def draw_compass_rose(ax, cx, cy, r, color, bg_color, font_props=None):
    ...
    secs = [(45, r_sec, w_sec), (315, r_sec, w_sec), (225, r_sec, w_sec), (135, r_sec, w_sec)]
...
def ensure_natural_earth_file():
    ...
    if not os.path.exists(geojson_path):
        ...
        print("Downloading Natural Earth 10m states and provinces GeoJSON (approx 38MB)...")
...
def fetch_boundary_data(region, admin_level):
    ...
    if is_major_country:
        ...
        if os.path.exists(geojson_path):
            ...
            try:
                print(f"Loading '{mapped_country}' states from offline Natural Earth database...")
                gdf_full = gpd.read_file(geojson_path)
                gdf_filtered = gdf_full[gdf_full["admin"].str.lower() == mapped_country.lower()]
                if not gdf_filtered.empty:
                    print(f"✓ Found {len(gdf_filtered)} states/provinces.")
                    return gdf_filtered
                else:
                    print(f"⚠ Country '{mapped_country}' not found in Natural Earth. Falling back to OSMnx.")
            except Exception as e:
    ...
    print(f"Downloading boundary features for '{region}' (admin_level={admin_level}) via OSMnx...")
    try:
        gdf = ox.features_from_place(region, tags={"boundary": "administrative", "admin_level": str(admin_level)})
        # Keep only Polygons / MultiPolygons
        gdf = gdf[gdf.geometry.type.isin(["Polygon", "MultiPolygon"])]
        if gdf.empty:
            raise ValueError("No administrative boundary polygons found for the specified region and level.")
...
def create_boundary_poster(
    ...
    if region.lower() in ["united states", "usa", "united states of america", "us"] and contiguous:
    ...
    gdf_proj["geometry"] = gdf_proj["geometry"].simplify(tolerance=simplify_tolerance, preserve_topology=True)
    ...
    for i, frac in enumerate(buffer_fractions):
        ...
        if isinstance(buffered, (Polygon, MultiPolygon)):
            ...
            for poly in polys:
                patch = patches.Polygon(np.array(poly.exterior.coords), facecolor=color, edgecolor='none', alpha=opacity, zorder=1.0 + i * 0.1)
                ax.add_patch(patch)
    ...
    card_border_inner = patches.Rectangle(
        (card_x + 0.003, card_y + 0.003), card_w - 0.006, card_h - 0.006,
        facecolor='none',
        edgecolor=INK_COLOR,
        linewidth=0.5,
        transform=ax.transAxes,
    ...
    if FONTS:
        ...
        font_main = FontProperties(family="serif", weight="bold", size=24 * scale_factor)
        font_sub = FontProperties(family="serif", style="italic", size=10 * scale_factor)
    ...
    # Center text lines
    ax.text(0.5, card_y + 0.055, display_title, color=text_color, ha='center', va='center', fontproperties=font_main, transform=ax.transAxes, zorder=23)
    ax.text(0.5, card_y + 0.030, display_subtitle, color=text_color, ha='center', va='center', fontproperties=font_sub, transform=ax.transAxes, zorder=23)
    ax.text(0.5, card_y + 0.012, coords_str, color=text_color, ha='center', va='center', fontproperties=font_coords, transform=ax.transAxes, zorder=23)

    # Save initial output
    if not output_file:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        region_slug = region.lower().replace(" ", "_").replace(",", "")
        output_file = os.path.join(POSTERS_DIR, f"{region_slug}_watercolor_{timestamp}.png")

    os.makedirs(POSTERS_DIR, exist_ok=True)
    plt.savefig(output_file, dpi=300, facecolor=DEEP_OCEAN, edgecolor='none', bbox_inches=None)
    plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--region", "-r", required=True, help="Region name (e.g., 'United States', 'India', 'Germany')")
    parser.add_argument("--admin-level", "-l", type=int, default=4, help="OSM administrative level (default: 4 for states)")
    parser.add_argument("--title", "-t", help="Title override (defaults to region name)")
    parser.add_argument("--subtitle", "-s", help="Subtitle override")
    parser.add_argument("--width", type=int, default=12, help="Poster width in inches (default: 12)")
    parser.add_argument("--height", type=int, default=16, help="Poster height in inches (default: 16)")
    parser.add_argument("--grain", type=float, default=0.36, help="Grain texture intensity (default: 0.36)")
    parser.add_argument("--contiguous", action="store_true", default=True, help="Filter to contiguous US states only if region is USA")
    parser.add_argument("--no-contiguous", action="store_false", dest="contiguous", help="Do not filter out Alaska/Hawaii for USA")
